...
def get_random_books(n=10): 
  ...
  book_df = reviews_raw[['Book_id', 'Book_name', 'Image_url']]
  book_df.rename(columns = {'Book_name':'book_name_long', 'Book_id':'book_id', 'Image_url':'image_url'}, inplace = True) 
...
def homepage():
    ...
    url_map = get_book_url_map()
    name_map = get_book_name_map()
    ...
    for row in df.values:
        userID = str(UserId)
        bookID = str(row[0])
        book_name = name_map[row[0]]['book_name'][:30]
        image_url = url_map[row[0]]['image_url']
        ...
        if not userID in keys_list:
            d['children'].append({"userID": userID, "children": [{"bookID": bookID, "book_name": book_name, "image_url": image_url}]})

        # if 'the_parent' IS a key in the flare.json, add a new child to it
        else:
            d['children'][keys_list.index(userID)]['children'].append({"bookID": bookID, "book_name": book_name, "image_url": image_url})
